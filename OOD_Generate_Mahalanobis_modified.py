...
def main():
    ...
    if args.dataset == 'svhn':
        out_dist_list = ['cifar10', 'lsun_resize']
    else:
        out_dist_list = ['svhn', 'lsun_resize']
    ...
    sample_mean, precision = lib_generation.sample_estimator_gmm(model, args.num_classes, feature_list, train_loader)
    import pickle
    with open('sample_mean_gmm.pkl', 'wb') as f:
        pickle.dump(sample_mean, f)
    with open('precision_gmm.pkl', 'wb') as f:
        pickle.dump(precision, f)
    
    # with open('sample_mean.pkl', 'rb') as f:
    #     sample_mean = pickle.load(f)
    # with open('precision.pkl', 'rb') as f:
    #     precision = pickle.load(f)
    print("shape of sample_mean and precision", len(sample_mean), len(precision))
    # assert False
    ...
    for magnitude in tqdm(m_list):
        ...
        for i in range(num_output):
            M_in = lib_generation.get_Mahalanobis_score_gmm(model, test_loader, args.num_classes, args.outf, \
        ...
        for out_dist in out_dist_list:
            ...
            for i in range(num_output):
                M_out = lib_generation.get_Mahalanobis_score_gmm(model, out_test_loader, args.num_classes, args.outf, \
            ...
            file_name = os.path.join(args.outf, 'Mahalanobis_gmm_%s_%s_%s.npy' % (str(magnitude), args.dataset , out_dist))
