...
def main():
    ...
    if args.validation_src == 'FGSM':
        if args.dataset == 'svhn':
            out_dist_list = ['lsun_resize', 'FGSM']
        else:
            out_dist_list = ['svhn','lsun_resize', 'FGSM']
    else:
        if args.dataset == 'svhn':
            out_dist_list = ['lsun_resize']
        else:
    ...
    with open('./feature_lists/feature_lists_{}_lsun_resize_{}_Wlinear.pickle'.format(args.net_type, args.dataset), 'rb') as f:
